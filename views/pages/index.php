<header class="header">
    
    <div class="header__text-box">
        <h1 class="main-heading">
            <span class="main-heading--main">Travelling</span>
            <span class="main-heading--sub">The way you imagine</span>
        </h1>
        <a href="#" class="btn btn--white btn--animated">View our tours</a>
    </div>
</header>

<main>

    <section class="section-about">
        <div class="u-center-text u-mb-big">
            <h2 class="heading-secondary">
                Exciting Tour for Adventorous people
            </h2>
        </div>
        <div class="row">
            <div class="col-1-of-2">
                <h3 class="heading-tertiary u-mb-small">You are going to fall for these views</h3>
                <p class="paragraph">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Ab unde rem molestiae libero quas dignissimos nesciunt corporis magni obcaecati quo, architecto quis, ea blanditiis quisquam facilis sit praesentium quibusdam sapiente?
                </p>
                <h3 class="heading-tertiary u-mb-small">Live the adventure of your life</h3>
                <p class="paragraph">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Ab unde rem molestiae libero quas dignissimos nesciunt corporis magni obcaecati quo, architecto quis.
                </p>
                <a class="btn btn--purple u-mt-small" href="#">Discover more</a>
            </div>
            <div class="col-1-of-2">
                <div class="composition">
                    <img src="./img/landscape-portrait-sea.jpg" alt="" class="composition__img composition__img--p1">
                    <img src="./img/landscape-sea-tower.jpg" alt="" class="composition__img composition__img--p2">
                    <img src="./img/landscape-sunset.jpg" alt="" class="composition__img composition__img--p3">
                </div>
            </div>
        </div>
    </section>

    <section class="section-features">
        <div class="row">
            <div class="col-1-of-3">
                <div class="feature-box">
                    <i class="feature-box__icon fa fa-heart"></i>
                    <h3 class="heading-tertiary">Explore the world</h3>
                    <p class="feature-box_text">
                        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Explicabo, qui ullam. Sequi consequatur officiis ipsam voluptatem? Recusandae, laborum cupiditate ipsam delectus error libero, a eligendi quos molestias odio dicta nemo.
                    </p>
                </div>
            </div>
            <div class="col-1-of-3">
                <div class="feature-box">
                    <i class="feature-box__icon fa fa-heart"></i>
                    <h3 class="heading-tertiary">Explore the world</h3>
                    <p class="feature-box_text">
                        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Explicabo, qui ullam. Sequi consequatur officiis ipsam voluptatem? Recusandae, laborum cupiditate ipsam delectus error libero, a eligendi quos molestias odio dicta nemo.
                    </p>
                </div>
            </div>
            <div class="col-1-of-3">
                <div class="feature-box">
                    <i class="feature-box__icon fa fa-heart"></i>
                    <h3 class="heading-tertiary">Explore the world</h3>
                    <p class="feature-box_text">
                        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Explicabo, qui ullam. Sequi consequatur officiis ipsam voluptatem? Recusandae, laborum cupiditate ipsam delectus error libero, a eligendi quos molestias odio dicta nemo.
                    </p>
                </div>
            </div>
        </div>
    </section>

    <section class="section-tours">
        <div class="u-center-text u-mb-big">
            <h2 class="heading-secondary">
                Exciting Tour for Adventorous people
            </h2>
        </div>
        <div class="row">
            <div class="col-1-of-3">
                <div class="card">
                    <div class="card__side card__side--front">
                        <div class="card__picture card__picture--1">

                        </div>
                        <h4 class="card__heading">
                            <span class="card__heading-span card__heading-span--1">The Green coastline</span>
                        </h4>
                        <div class="card__details">
                            <ul>
                                <li>4 day tour</li>
                                <li>Amazing landscapes</li>
                                <li>4* Hotels</li>
                                <li>Tour guide included</li>
                                <li>Maximum 10 people</li>
                            </ul>
                        </div>
                    </div>
                    <div class="card__side card__side--back card__side--back-1">
                        <div class="card__callta">
                            <div class="card__price-box">
                                <p class="card__price-only">only</p>
                                <p class="card__price-value">€349</p>
                            </div>
                            <a href="" class="btn btn--white">Book now</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-1-of-3">
                <div class="card">
                    <div class="card__side card__side--front">
                        <div class="card__picture card__picture--2">

                        </div>
                        <h4 class="card__heading">
                            <span class="card__heading-span card__heading-span--2">The Green coastline</span>
                        </h4>
                        <div class="card__details">
                            <ul>
                                <li>4 day tour</li>
                                <li>Amazing landscapes</li>
                                <li>4* Hotels</li>
                                <li>Tour guide included</li>
                                <li>Maximum 10 people</li>
                            </ul>
                        </div>
                    </div>
                    <div class="card__side card__side--back card__side--back-2">
                        <div class="card__callta">
                            <div class="card__price-box">
                                <p class="card__price-only">only</p>
                                <p class="card__price-value">€349</p>
                            </div>
                            <a href="" class="btn btn--white">Book now</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-1-of-3">
                <div class="card">
                    <div class="card__side card__side--front">
                        <div class="card__picture card__picture--3">

                        </div>
                        <h4 class="card__heading">
                            <span class="card__heading-span card__heading-span--3">The Green coastline</span>
                        </h4>
                        <div class="card__details">
                            <ul>
                                <li>4 day tour</li>
                                <li>Amazing landscapes</li>
                                <li>4* Hotels</li>
                                <li>Tour guide included</li>
                                <li>Maximum 10 people</li>
                            </ul>
                        </div>
                    </div>
                    <div class="card__side card__side--back card__side--back-3">
                        <div class="card__callta">
                            <div class="card__price-box">
                                <p class="card__price-only">only</p>
                                <p class="card__price-value">€349</p>
                            </div>
                            <a href="" class="btn btn--white">Book now</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</main>